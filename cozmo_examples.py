...
def cozmo_program(robot: Rob.robot.Robot):
...
def cozmo_program(robot: Rob.robot.Robot):
    ...
    if cube1 is not None:
        cube1.set_lights(Rob.lights.red_light)
    else:
        Rob.logger.warning("Cozmo is not connected to a LightCube1Id cube - check the battery.")

    if cube2 is not None:
        cube2.set_lights(Rob.lights.green_light)
    else:
        Rob.logger.warning("Cozmo is not connected to a LightCube2Id cube - check the battery.")

    if cube3 is not None:
        cube3.set_lights(Rob.lights.blue_light)
    else:
        Rob.logger.warning("Cozmo is not connected to a LightCube3Id cube - check the battery.")
...
def cozmo_program(robot: Rob.robot.Robot):
    ...
    robot.play_anim_trigger(Rob.anim.Triggers.CubePounceLoseSession).wait_for_completed()
...
async def dock_with_cube(robot: Rob.robot.Robot):
    ...
    await robot.dock_with_cube(cube, approach_angle=Rob.util.degrees(180), num_retries=2).wait_for_completed()



Rob.run_program(cozmo_program)
